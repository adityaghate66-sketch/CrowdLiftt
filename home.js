...
const SUPABASE_URL = 'https://ndcvzlmyitfmbkiigpoe.supabase.co';
const SUPABASE_KEY = 'sb_publishable_q-u-LN1c8080T-_rIFHosw_a55lFblX';
...
function setGreeting() {
  ...
  document.getElementById('greetingSub').textContent =
    h < 12 ? 'Start strong today.' : h < 17 ? 'Keep pushing.' : 'Evening grind hits different.';
  const timeStr = new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
  document.getElementById('greetingTime').textContent = timeStr;
}
...
// ── SUPABASE CROWD FUNCTIONS ──────────────────────────────────
async function getCrowdFromSupabase() {
  try {
    const { data, error } = await supabase
      .from('crowd')
      .select('muscle, count');
    if (error) throw error;
    // convert array to object — reduce HOF
    return data.reduce((acc, row) => {
      acc[row.muscle] = row.count;
      return acc;
    }, {});
  } catch (err) {
    console.error('Error fetching crowd:', err);
    return {};
  }
}

async function updateCrowdInSupabase(muscle, delta) {
  try {
    // Check if row exists
    ...
    if (data) {
      // Update existing row
      const newCount = Math.max(0, (data.count || 0) + delta);
      await supabase
        .from('crowd')
      ...
      await supabase
        .from('crowd')
        .insert({ muscle, count: Math.max(0, delta) });
    }
  } catch (err) {
...
// ── RENDER CROWD GRID ─────────────────────────────────────────
async function renderCrowdGrid() {
  const crowd   = await getCrowdFromSupabase();
  const muscles = ['Chest', 'Back', 'Arms', 'Legs', 'Shoulders', 'Core'];
  const grid    = document.getElementById('crowdGrid');
...
supabase
  .channel('crowd-changes')
  .on('postgres_changes', { event: '*', schema: 'public', table: 'crowd' }, () => {
    renderCrowdGrid(); // auto refresh when any user checks in/out
  })
  .subscribe();
...
document.getElementById('checkinBtn').addEventListener('click', async () => {
  ...
  // Update Supabase crowd count for each muscle — real-time for ALL users
  await Promise.all(selectedMuscles.map(m => updateCrowdInSupabase(m, +1)));
...
document.getElementById('checkoutBtn').addEventListener('click', async () => {
  ...
  // Decrease crowd in Supabase for each muscle
  await Promise.all(muscles.map(m => updateCrowdInSupabase(m, -1)));
...
function buildHeatmap() {
  ...
  slots.forEach(t => {
    ...
    const {hour, period} = formatHour(t);
    lbl.textContent = `${hour}${period[0].toLowerCase()}`;
    hm.appendChild(lbl);
    for (let d = 0; d < 7; d++) {
      const lvl  = crowdData[d][t - 5] || 1;
      const cell = document.createElement('div');
      cell.className        = 'hm-cell';
      cell.style.background = HMCOLS[lvl];
      hm.appendChild(cell);
    }
  });
}
...
async function fetchWeather() {
  try {
    ...
    const match = tips.filter(t => t.codes.includes(code));
    const tip   = match.length > 0 ? match[0].tip : temp > 35 ? 'Hot outside — gym busier than usual.' : 'Check crowd before heading out.';
    document.getElementById('weatherTemp').textContent = `${temp}°C`;
    document.getElementById('weatherDesc').textContent = desc;
    document.getElementById('weatherTip').textContent  = tip;
